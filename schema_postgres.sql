-- MintMotive Ops database schema (PostgreSQL)
-- Generated from schema.sql (the SQLite version) — keep both in sync when
-- adding/changing tables. Differences from the SQLite version:
--   * INTEGER PRIMARY KEY AUTOINCREMENT -> SERIAL PRIMARY KEY
--   * DEFAULT CURRENT_TIMESTAMP on a TEXT column needs an explicit cast to
--     text (Postgres won't implicitly cast timestamp -> text the way
--     SQLite does), so those defaults are written as
--     to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS') — formatted to
--     match SQLite's own CURRENT_TIMESTAMP output so both backends store
--     dates in the same TEXT shape.
--   * company_settings is moved after client_payment_terms/
--     supplier_payment_terms since Postgres (unlike SQLite) requires a
--     REFERENCES target to already exist at CREATE TABLE time.

-- =========================================================================
-- Admin config tables (company_settings itself is further down, after
-- client_payment_terms/supplier_payment_terms which it references)
-- =========================================================================
...
-- =========================================================================
-- Company / Admin singleton row (kept here, not at the top, because it
-- references client_payment_terms/supplier_payment_terms above)
-- =========================================================================
...
CREATE TABLE locations (
    ...
    -- Gridfinity bin coordinates — see schema.sql for the full comment.
    baseplate_name TEXT,
    grid_x INTEGER,
    grid_y INTEGER
);
...
CREATE TABLE sourcing_prospects (
    ...
    date_added TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE TABLE firmware_versions (
    ...
    release_notes TEXT,
    released_at TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE TABLE power_readings (
    ...
    watts REAL NOT NULL,
    recorded_at TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS')
);
...
CREATE TABLE sales_orders (
    ...
    order_date TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE TABLE expenses (
    id SERIAL PRIMARY KEY,
    expense_date TEXT NOT NULL DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE TABLE quotes (
    ...
    client_id INTEGER NOT NULL REFERENCES clients(id) ON DELETE RESTRICT,
    quote_date TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
    expiry_date TEXT,
    payment_term_id INTEGER REFERENCES client_payment_terms(id) ON DELETE SET NULL,
...
CREATE TABLE invoices (
    ...
    invoice_date TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
    due_date TEXT,
    payment_term_id INTEGER REFERENCES client_payment_terms(id) ON DELETE SET NULL,
...
CREATE TABLE rmas (
    ...
    received_date TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
    resolved_date TEXT,
    notes TEXT,
    created_at TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS')
);
...
CREATE TABLE firmware_flash_log (
    ...
    log_output TEXT,
    flashed_at TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS')
);
...
CREATE TABLE documents (
    ...
    upload_date TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
    notes TEXT
);
...
CREATE TABLE audit_logs (
    ...
    changed_by TEXT,
    changed_at TEXT DEFAULT to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS'),
    details TEXT
);
